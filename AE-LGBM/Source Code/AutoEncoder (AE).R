...
feat_a<-as.matrix(read.csv("../Independent Species/Ecoli/total_features_a.csv"))
feat_b<-as.matrix(read.csv("../Independent Species/Ecoli/total_features_b.csv"))

k<-202 #Number of nodes in hidden layer
...
####################Save Model
model_AE1 %>% save_model_hdf5(paste0("AE_Yeast_a_.h5", collapse = ""))
model_AE2 %>% save_model_hdf5(paste0("AE_Yeast_b_.h5", collapse = ""))

##############
gbm_data<-cbind(red.feat.a,red.feat.b)
gbm_data<-as.data.frame(cbind(gbm_data, label))
write.csv(gbm_data[sample(1:nrow(gbm_data)),], "gbm_Yeast.csv", row.names = F) #shuffle and save
