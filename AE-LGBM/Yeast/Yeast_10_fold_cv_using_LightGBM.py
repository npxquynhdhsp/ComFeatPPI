...
Mydata = pd.read_csv("gbm_Yeast_enc.csv", header=0)  # gbm_Yeast là đặc trưng đã qua AutoEncoder
print(Mydata)
Mydata.columns.name = None

data = Mydata.drop(Mydata.columns[Mydata.shape[1] - 1],
                   axis=1)
label = Mydata.iloc[:, (Mydata.shape[1] - 1)]

part = np.repeat(list(range(0, 10)),
                 math.floor((data.shape[0] / 10)),
                 axis=0)  # partition for 10 fold cross validation

result = None
result2 = None
val50 = []
val_acc = []
cv = StratifiedKFold(n_splits=10, random_state=42, shuffle=True)  # thnhan
...
for k in range(10):
    ...
    val50.append(np.sum(pd.DataFrame(result2).iloc[0, :]))
    val_acc.append(Acc)

print(f"ACC {np.mean(val_acc)} {np.std(val_acc)}")
