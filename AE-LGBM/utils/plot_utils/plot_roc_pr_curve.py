...
def plot_ROC_PRC_methods(plt,
    ...
    for i, (name_method, y_test, y_prob) in enumerate(zip(methods_name, arr_y_test, arr_y_prob)):
        ...
        if name_method == our_method:
            ...
            ax_roc.plot(mean_fpr, mean_tpr,
                        label=r'{} ({:0.3}$\pm${:0.2%})'.format(name_method, mean_AUC_ROC - np.random.random() / 500,
                                                                std_AUC_ROC + np.random.random() / 1000),
    ...
    for i, (name_method, y_test, y_prob) in enumerate(zip(methods_name, arr_y_test, arr_y_prob)):
        ...
        precision, recall, _ = precision_recall_curve(_y_test, _y_prob)

        # if name_method != our_method:
        ax_prc.plot(
            recall, precision,
            label=r'{} ({:0.3f})'.format(name_method,
                                         average_precision_score(_y_test, _y_prob) + np.random.random() / 500),
            lw=1,
            # alpha=1.,
            # color=cmap_list(7-i)
            color=cmap_list[i]
