...
def calculate_scores(true_y, prob_y, pred_y):
    ...
    if len(prob_y.shape) > 2:
        ...
        scores.update({'AUC': roc_auc_score(true_y, prob_y)})
        scores.update({'AUPR': average_precision_score(true_y, prob_y)})
...
def print_metrics(true_y, prob_y=None, pred_y=None, metrics=None, verbose=1):
    ...
    # print(true_y)
    # if true_y.shape < prob_y.shape:
    #     # true_y = to_categorical(true_y)
    #     scores = calculate_scores(true_y, prob_y[:, 1], pred_y)
    # else:
