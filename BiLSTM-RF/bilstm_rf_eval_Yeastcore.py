...
def eval_Yeastcore(X_a, X_b, y):
    ...
    for ii, (tr_ii, te_ii) in enumerate(skf.split(X_a, y)):
        ...
        # rf_cls = LGBMClassifier(n_estimators=100, random_state=0)
        rf_cls = XGBClassifier(n_estimators=100)

        rf_cls.fit(tr_X, tr_y)
        prob_y = rf_cls.predict_proba(te_X)

        method_result['fold' + str(ii)] = {"true": te_y, "prob": prob_y}
        pickle.dump(method_result, open("BiLSTM_RF_eval_Yeastcore.pkl", "wb"))
