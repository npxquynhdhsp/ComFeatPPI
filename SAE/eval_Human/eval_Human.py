...
def eval_Human_AC(args):
    os.chdir(args.human_dset)
    n_pos, n_neg = args.human_size

    print("\n---", os.getcwd())

    if not os.path.exists('AC_a.csv'):
        print('Wait')
        AC_a, AC_b = get_AC(args.human_dset)
    else:
        AC_a = pd.read_csv('AC_a.csv').values
        AC_b = pd.read_csv('AC_b.csv').values
        print(AC_a.shape, AC_b.shape)

    # if os.path.exists(dset + '/CT_a.csv'):
    #     print("RUN file CT.R first")
    #     exit(1)
    ...
    for ii, (tr_ii, te_ii) in enumerate(skf.split(X, y)):
        ...
        pickle.dump(method_result, open("SAE_eval_Human.pkl", "wb"))
