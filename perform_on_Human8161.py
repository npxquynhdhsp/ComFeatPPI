...
def eval_model(pairs, labels):
    ...
    for i, (tr_inds, te_inds) in enumerate(skf.split(pairs, labels)):
        ...
        # ====== DEF MODEL
        if os.path.exists('trained_models/OurModel_trained_on_Human8161_fold' + str(i) + '.h5'):
            model = load_model('trained_models/OurModel_trained_on_Human8161_fold' + str(i) + '.h5')
            print("====== LOADED MODEL")
        else:
            ...
            # ====== SAVE MODEL
            model.save("OurModel_trained_on_Human8161_fold" + str(i) + ".h5")
        ...
        method_result['fold' + str(i)] = {"true_y": np.argmax(te_Y, axis=1),
                                          "prob_Y": prob_Y}
        pickle.dump(method_result, open(r'trained_models/OurModel_trained_on_Human8161_fold.pkl', 'wb'))
        # ======
...
handdim = 880  # handcrafted feature dimension, # 650
