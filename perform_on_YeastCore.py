...
from extracted_handcracted_features.load_handfeat import load_handfeat_YeastCore
...
def prepare_YeastCore_feat(w2v_model, protlen, dset):
    ...
    pos_feat_A, pos_feat_B, neg_feat_A, neg_feat_B = load_handfeat_YeastCore("extracted_handcracted_features/Yeastcore")
...
def eval_model(pairs, labels):
    ...
    for i, (tr_inds, te_inds) in enumerate(skf.split(pairs, labels)):
        ...
        # ====== get 4 extracted_handcracted_features
        ...
        # ====== DEF MODEL
        if os.path.exists('trained_models/OurModel_trained_on_Yeastcore_fold' + str(i) + '.h5'):
            model = load_model('trained_models/OurModel_trained_on_Yeastcore_fold' + str(i) + '.h5')
        else:
        ...
        method_result['fold' + str(i)] = {"true_y": np.argmax(te_Y, axis=1),
                                          "prob_Y": prob_Y}
        pickle.dump(method_result, open(r'trained_models/OurModel_trained_on_Yeastcore_fold.pkl', 'wb'))
        # ======
...
handdim = 880  # handcrafted feature dimension
